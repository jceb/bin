...
# @Created      : 2006-12-24
# @Last Modified: Sun 19. Dec 2010 11:45:05 +0100 CET
# @Revision     : 0.0
...
MAIL_PREV_INIT_STRING = r'EMAIL;TYPE=PREF:'
...
class vcard:
    ...
    def setPrintFormat(self, format):
        if format == None or (type(format) in (str, unicode) and len(format) > 0):
            self.__printFormat = format
...
def getAllEmailAddresses(vcards, ignored):
    lines = []
    mail_pref_re = re.compile(r'^' + MAIL_PREV_INIT_STRING + r'(.*)$', re.MULTILINE)
...
def usage():
    ...
    print "\t-e|--email name|email to match to"
    sys.exit(1)

# main program starts here
if __name__ == '__main__':
    try:
		optlist, args = getopt.getopt(sys.argv[1:], "c:b:e:v:f:", ["allemailaddresses", "allbirthdays", "category=", 'vcardsbycategory=', "birthday=", "email=", "mutt_query=", "file"])
